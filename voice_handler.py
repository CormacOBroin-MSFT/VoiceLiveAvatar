...
class VoiceSessionHandler:
    ...
    async def _setup_session(self, connection):
        ...
        # Proactive greeting / priming logic depends on avatar mode:
        ...
        proactive = config.get("enableProactive", True)
        priming_enabled = bool(
            config.get("enablePriming", False)
            and (config.get("primingMessage") or "").strip()
        )
        should_respond = proactive or priming_enabled

        if not config.get("avatarEnabled", False):
            if should_respond:
                try:
                    if priming_enabled:
                        await self._send_priming_message(connection)
                    logger.info("[SEND] response.create (initial response, no avatar)")
            ...
            if should_respond:
                try:
                    if priming_enabled:
                        await self._send_priming_message(connection)
                    logger.info("[SEND] response.create (initial response, websocket avatar)")
                    await connection.response.create()
                    logger.info("Initial response sent (websocket avatar)")
                except Exception as e:
                    logger.error(f"Failed to send initial response: {e}")
        else:
            # WebRTC avatar: defer until avatar connect
            self._pending_proactive = should_respond
            self._pending_priming = priming_enabled
            ...
            if event_type == ServerEventType.RESPONSE_AUDIO_DELTA:
                if hasattr(event, "delta") and event.delta:
                    ...
                    if self._audio_delta_count <= 3 or self._audio_delta_count % 100 == 0:
                        logger.info(f"[SEND->browser] audio_data chunk #{self._audio_delta_count} bytes={len(event.delta)} b64={len(audio_b64)}")
                    await self.send_message({
                        "type": "audio_data",
                        "data": audio_b64,
                        "format": "pcm16",
            ...
            # Avatar WebRTC signaling
                ...
                if server_sdp:
                    ...
                    if getattr(self, "_pending_proactive", False):
                        self._pending_proactive = False
                        pending_priming = getattr(self, "_pending_priming", False)
                        self._pending_priming = False
                        try:
                            if pending_priming:
                                await self._send_priming_message(connection)
                            logger.info("[SEND] response.create (initial response, after avatar connect)")
                            await connection.response.create()
                            logger.info("Initial response sent after avatar connect")
                        except Exception as e:
                            logger.error(f"Failed to send initial response: {e}")
    ...
    async def _send_priming_message(self, connection):
        """Insert the configured priming message as a user item before a proactive response."""
        priming = (self.config.get("primingMessage") or "").strip()
        if not priming:
            return
        try:
            from azure.ai.voicelive.models import (
            ...
            item = UserMessageItem(content=[InputTextContentPart(text=priming)])
            await connection.conversation.item.create(item=item)
            logger.info(f"[SEND] priming user message ({len(priming)} chars)")
        except Exception as e:
            logger.error(f"Failed to send priming message: {e}")
