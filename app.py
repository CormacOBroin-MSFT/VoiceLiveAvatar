...
async def get_config():
    ...
    def _read_default_file(filename: str) -> str:
        path = os.path.join(os.path.dirname(__file__), "defaults", filename)
        try:
            ...
            logger.warning(f"Failed to read defaults/{filename}: {exc}")
            return ""
    ...
    instructions = os.getenv("VOICELIVE_INSTRUCTIONS") or _read_default_file("instructions.txt")
    priming_message = os.getenv("VOICELIVE_PRIMING_MESSAGE") or _read_default_file("priming.json")
    ...
    return {
        ...
        "voiceSpeed": int(os.getenv("VOICELIVE_VOICE_SPEED", "100")),
        "avatarEnabled": _env_bool("VOICELIVE_AVATAR_ENABLED", True),
        "enableProactive": _env_bool("VOICELIVE_ENABLE_PROACTIVE", False),
