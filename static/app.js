...
async function fetchServerConfig() {
    try {
        ...
        if (typeof config.voiceSpeed === 'number') {
            const el = document.getElementById('voiceSpeed');
            el.value = String(config.voiceSpeed);
            el.dispatchEvent(new Event('input'));
        }
        if (typeof config.avatarEnabled === 'boolean') {
            document.getElementById('avatarEnabled').checked = config.avatarEnabled;
        }
        if (typeof config.enableProactive === 'boolean') {
            document.getElementById('enableProactive').checked = config.enableProactive;
        }
        if (typeof config.enablePriming === 'boolean') {
            document.getElementById('enablePriming').checked = config.enablePriming;
        }
...
function updateConditionalFields() {
    ...
    show('primingMessageField', !isAgent);
    ...
    // Entra token only shown for agent modes
...
function gatherConfig() {
    ...
    const config = {
        ...
        primingMessage: document.getElementById('primingMessage').value,
        enablePriming: document.getElementById('enablePriming').checked,
...
// Plays PCM audio deltas streamed from the Voice Live API.
// Only used when the avatar is OFF (avatar mode plays audio via WebRTC track).
...
function handleAudioDelta(base64Data) {
    ...
    // permits creation in the running state.
    if (!playbackContext || playbackContext.state === 'closed') {
        playbackContext = new AudioContext({ sampleRate: 24000 });
        // Analyser MUST belong to playbackContext. Connecting nodes across
        // AudioContexts throws InvalidAccessError and silently drops audio.
    ...
    if (audioDeltaCount <= 3 || audioDeltaCount % 100 === 0) {
        console.log(`[Audio] chunk #${audioDeltaCount} samples=${float32.length} ctx=${playbackContext.state} now=${now.toFixed(3)} schedAt=${nextPlaybackTime.toFixed(3)}`);
    }
...
function preparePeerConnection(iceServers) {
    ...
    // Handle incoming tracks (separate <video> and <audio> elements per MS sample)
    pc.ontrack = (event) => {
        console.log(`[WebRTC/prepare] ontrack kind=${event.track.kind} id=${event.track.id} streams=${event.streams.length}`);
        const container = document.getElementById('avatarVideo');
        ...
        if (event.track.kind === 'video') {
            el.playsInline = true;
            avatarVideoElement = el;
        }
        if (container) container.appendChild(el);
        el.onplaying = () => console.log(`[WebRTC/prepare] ${event.track.kind} onplaying`);
        el.onerror = (e) => console.error(`[WebRTC/prepare] ${event.track.kind} element error:`, e);
    };

    pc.oniceconnectionstatechange = () => {
        console.log(`[WebRTC/prepare] iceConnectionState=${pc.iceConnectionState}`);
    };
    pc.onconnectionstatechange = () => {
        console.log(`[WebRTC/prepare] connectionState=${pc.connectionState}`);
    };
...
function setupWebRTC(iceServers) {
    ...
    peerConnection.ontrack = (event) => {
        console.log(`[WebRTC] ontrack kind=${event.track.kind} id=${event.track.id} streams=${event.streams.length}`);
        const stream = event.streams[0];
        const el = document.createElement(event.track.kind);
        el.id = `avatar-${event.track.kind}`;
        el.autoplay = true;
        ...
        if (container) container.appendChild(el);
        el.onplaying = () => console.log(`[WebRTC] ${event.track.kind} onplaying`);
        el.onerror = (e) => console.error(`[WebRTC] ${event.track.kind} element error:`, e);
    };
    ...
    peerConnection.oniceconnectionstatechange = () => {
        ...
        if ((s === 'failed' || s === 'disconnected') && !sawRelayCandidate) {
            addMessage('system',
                'Avatar WebRTC connection failed: your network blocked the TURN relay allocation. ' +
                'This typically happens behind corporate VPNs/proxies that route through Azure. ' +
                'Switch the avatar output mode to "WebSocket" and reconnect.');
        }
    };
    peerConnection.onconnectionstatechange = () => {
        console.log(`[WebRTC] connectionState=${peerConnection.connectionState}`);
        if (peerConnection.connectionState === 'connected') {
            // Poll RTP stats once to confirm media is actually flowing
            setTimeout(() => {
                peerConnection.getStats(null).then(stats => {
                    stats.forEach(report => {
                        if (report.type === 'inbound-rtp') {
                            console.log(`[WebRTC] inbound-rtp kind=${report.kind} bytesReceived=${report.bytesReceived} packetsReceived=${report.packetsReceived}`);
                        }
                    });
                });
            }, 3000);
    ...
    peerConnection.onicecandidate = (event) => {
        if (event.candidate) {
            // Log each candidate to see if TURN relay candidates are arriving
            const c = event.candidate.candidate || '';
            const typeMatch = c.match(/typ (\w+)/);
            const typ = typeMatch ? typeMatch[1] : '?';
            if (typ === 'relay') sawRelayCandidate = true;
            console.log(`[WebRTC] ICE candidate typ=${typ} -- ${c.substring(0, 80)}`);
            return;
        }
        if (!iceGatheringDone) {
            iceGatheringDone = true;
    ...
    peerConnection.createOffer().then(offer => {
        ...
        // Timeout fallback: send SDP after 20 seconds if ICE gathering hasn't completed.
        // TURN allocation can be slow; we want relay candidates included whenever possible.
        setTimeout(() => {
            if (!iceGatheringDone) {
                iceGatheringDone = true;
                const sdpJson = JSON.stringify(peerConnection.localDescription);
                const sdpBase64 = btoa(sdpJson);
                console.warn('[SDP] Sending base64 SDP (TIMEOUT - no relay candidate gathered, ICE likely to fail)');
                console.log('[SDP] starts with:', sdpBase64.substring(0, 40));
                ws.send(JSON.stringify({ type: 'avatar_sdp_offer', clientSdp: sdpBase64 }));
